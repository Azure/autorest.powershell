...
export /* @internal */ class Inferrer {
    ...
    for (const operationGroup of values(model.operationGroups)) {
      ...
      */
      if (this.isAzure
        ...
        if (getOperation && schema && [...values(getOperation?.responses), ...values(getOperation?.exceptions)].filter(each => each.protocol?.http?.statusCodes[0] !== 'default' && (<SchemaResponse>each).schema !== schema).length === 0) {
  ...
  async addCommandOperation(vname: string, parameters: Array<Parameter>, operation: Operation, variant: CommandVariant, state: State, preOperations: Array<Operation> | undefined, commandOperationType?: CommandOperationType): Promise<CommandOperation> {
    ...
    // Problem is OperationType.Update was not applied to Set cmdlets before: operation.requests[0].protocol.http?.method === 'patch' && variant.verb.toLowerCase() === 'update'
