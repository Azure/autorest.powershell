    ...
    try {
      ...
      switch (mediaType) {
        ...
        case KnownMediaType.QueryParameter: {
          ...
          return toExpression(`(null != ${value}  && ${value}.Count > 0 ? "${formatSerializedName}" + ${this.encode}(global::System.Linq.Enumerable.Aggregate(${value}, (current, each) => current + "," + ( null == each ? ${System.String.Empty} : each.ToString()) )) : ${System.String.Empty})`);
        }
