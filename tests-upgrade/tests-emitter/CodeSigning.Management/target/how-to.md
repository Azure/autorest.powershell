# How-To
This document describes how to develop for `Az.CodeSigning`.

## Building `Az.CodeSigning`
...
There are multiple scripts created for performing different actions for developing `Az.CodeSigning`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.CodeSigning.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.CodeSigning.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.CodeSigning`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.CodeSigning` into the session.
