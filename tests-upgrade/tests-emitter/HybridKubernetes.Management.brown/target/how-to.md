# How-To
This document describes how to develop for `Az.HybridKubernetes`.

## Building `Az.HybridKubernetes`
...
There are multiple scripts created for performing different actions for developing `Az.HybridKubernetes`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.HybridKubernetes.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.HybridKubernetes.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.HybridKubernetes`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.HybridKubernetes` into the session.
