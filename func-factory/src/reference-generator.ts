...
export function generateTS(ga: GraphContext, procs: ProcImplementations, flavor: GenerationFlavor = GenerationFlavor.ContInlineProc): string {

  const getSymbol = (x: SymbolSource | undefined) => x && ga.getSymbolFromSource(x);
  ...
  const def = (impl: ProcImplementation, proc: Proc, args: { [id: string]: string }, cb: { [id: string]: string }) => {
  ...
  // TS emit
  const tsType = (t: Type): string => t.name || "undefined";
  const tsValue = (x: JSONValue): string => JSON.stringify(x);

  const tsProcArgType = (p: { readonly [id: string]: { type: Type } | undefined }): string => `{ ${Object.entries(p).filter(x => x[1] !== undefined).map(x => `${x[0]}: ${tsType((x[1] || errorUnreachable()).type)}`).join(', ')} }`;
  const tsProcArgCbType = (p: Proc): string => `{ ${Object.entries(p.outputFlows).filter(x => x[1] !== undefined).map(x => `${x[0]}: (args: ${tsProcArgType(x[1] || errorUnreachable())}) => ${asyncVoid}`).join(', ')} }`;
  const tsProcArgs = (p: Proc): { [id: string]: string } => objMap(x => x, p.inputs);
  const tsProcArgsCb = (p: Proc): { [id: string]: string } => objMap(x => x, p.outputFlows);
  ...
  for (const x of Object.entries(ga.graph.inputs)) {
    const v = x[1];
    if (v !== undefined) result.push(`  const ${x[0]}: ${tsType(v.type)} = ${v.value !== undefined ? tsValue(v.value) : `args.${x[0]}`};`);
  }
  // prepare signatures for all data sinks
  type ControlSinkSig = { id: string, symbols: { id: string, type: Type, symbol: Set<SymbolInstance> }[] };
  const sinkSignatures = new Map<ControlSink, ControlSinkSig>();
  for (const sink of controlSinks) {
    if (sinkSignatures.has(sink)) continue;
    let symbols: { id: string, type: Type, symbol: Set<SymbolInstance> }[] = [];
    if (sink.type === "phi") {
      ...
      for (const entry of Object.entries(merge)) {
        ...
        if (val !== undefined) {
          ...
          symbols.push({ id: entry[0], type: outSym.type, symbol: new Set<SymbolInstance>(Object.values(val.sources).map(getSymbol).filter(x => x) as any).add(outSym) });
        }
      }
    }
    for (const symbol of ga.getDemand(sink)) {
      ...
      symbols.push({ id, symbol: new Set<SymbolInstance>([symbol]), type: symbol.type });
    }
  ...
  for (const n of nodesPhi) {
    ...
    const getSymbolId = (sym: Set<SymbolInstance>): string => "args." + ([...sym].map(x => sig.symbols.find(y => y.symbol.has(x))).filter(x => x !== undefined)[0] || { id: "/**/" }).id;
    result.push(`  const ${sig.id} = ${asyncModifier}(args: { ${sig.symbols.map(x => `${x.id}: ${tsType(x.type)}`).join(', ')} }): ${asyncVoid} => ${targetSig.id}({ ${targetSig.symbols.map(x => `${x.id}: ${getSymbolId(x.symbol)}`).join(', ')} });`);
  }
  for (const n of nodesProc) {
    const sink = controlSinks.find(x => x.type === "proc" && x.node === n) || errorUnreachable();
    const sig = sinkSignatures.get(sink) || errorUnreachable();
    const getSymbolId = (sym: Iterable<SymbolInstance>): string => "args." + ([...sym].map(x => sig.symbols.find(y => y.symbol.has(x))).filter(x => x !== undefined)[0] || { id: "/**/" }).id;
    const proc = ga.procs[n.procID];
    const procDef = procs[n.procID];
    if (proc === undefined || procDef === undefined) {
      result.push(`  const ${sig.id} = ${asyncModifier}(${sig.symbols.map(x => `${x.id}: ${tsType(x.type)}`).join(', ')}): ${asyncVoid} => /**/;`);
      continue;
    }
    ...
    for (const x of Object.entries(proc.outputFlows)) {
      ...
      const getSymbolIdX = (sym: Iterable<SymbolInstance>, args: { [id: string]: string }): string => {
    ...
    // else
    result.push(`  const ${sig.id} = ${asyncModifier}(args: { ${sig.symbols.map(x => `${x.id}: ${tsType(x.type)}`).join(', ')} }): ${asyncVoid} => ${procDef.defInline(argsInline1, argsInline2)};`);
  }
  ...
  if (initialFlow !== undefined) {
    ...
    const getSymbolId = (sym: Iterable<SymbolInstance>): string => lundef([...setIntersect(syms, sym)][0], x => x.source.id) || "/**/";
...
export function generateTS2(ga: GraphContext, procs: ProcImplementations, flavor: GenerationFlavor = GenerationFlavor.ContInlineProc): string {
  ...
  const getUniqueSymbolName = ((): ((symbol: SymbolInstance) => string) => {
    const names = new Map<SymbolInstance, string>();
    return (symbol: SymbolInstance): string => {
  ...
  for (const symbol of ga.symbols) {
    ...
    result.push(`  let ${getUniqueSymbolName(symbol)}: ${tsType(symbol.type)}${init || ""};${symbol.names.length === 0 ? "" : " // " + symbol.names.join(", ")}`);
  }

  // prepare signatures for all data sinks
  const sinkSignatures = new Map<ControlSink, { func: string, call: string }>();
