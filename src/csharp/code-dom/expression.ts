...
/** a c# 'is' expression */
export function Is(expression: Expression, isType: TypeDeclaration): Expression {
  return new IsExpression(expression, isType);
}

/** a c# 'is' expression */
export class IsExpression implements Expression {
  constructor(public expression: Expression, public isType: TypeDeclaration) {
  }
  get value(): string {
    return `${this.expression.value} is ${this.isType.declaration}`;
  }
}

/** a c# 'is' expression that declares a local variable  */
export function IsDeclaration(expression: Expression, isType: TypeDeclaration, name: string): IsExpressionDeclaration {
...
export class IsExpressionDeclaration extends LocalVariable implements Expression {
  constructor(public expression: Expression, public isType: TypeDeclaration, name: string) {
    super(name, isType);
  }
  public get check(): Expression {
    return new LiteralExpression(`${this.expression.value} is ${this.isType.declaration} ${this.name}`);
  }
