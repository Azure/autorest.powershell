import { ParameterLocation, NewResponse } from '#common/code-model/http-operation';
...
import { ClientRuntime, EventListener as EventListenr, ISendAsync, IEventListener } from '#csharp/lowlevel-generator/clientruntime';
...
import { Schema, HttpOperation } from '#csharp/lowlevel-generator/code-model';
...
export class OperationMethod extends Method {
  ...
  constructor(protected parent: Class, public operation: HttpOperation, protected state: State, objectInitializer?: Partial<OperationMethod>) {
    super(operation.details.csharp.name, mscorlib.Task());
    ...
    this.listenerParameter = this.addParameter(new Parameter("listener", IEventListener));
...
export class CallMethod extends Method {
  constructor(protected parent: Class, protected opMethod: OperationMethod, protected state: State, objectInitializer?: Partial<OperationMethod>) {
    super(`${opMethod.operation.details.csharp.name}_Call`, mscorlib.Task());
    this.apply(objectInitializer);
          ...
          yield Switch({ value: `_response.StatusCode` }, function* () {
            ...
            for (const { key: responseCode, value: responses } of items(opMethod.operation.responses_new)) {
              if (responseCode !== 'default') {
                yield Case(`(System.Net.HttpStatusCode)${responseCode}`, $this.responsesEmitter($this, opMethod, responses));
              } else {
                yield DefaultCase($this.responsesEmitter($this, opMethod, responses));
              }
            }
  ...
  private *responsesEmitter($this: CallMethod, opMethod: OperationMethod, responses: Array<NewResponse>) {
...
export class ValidationMethod extends Method {
  ...
  constructor(protected parent: Class, protected opMethod: OperationMethod, protected state: State, objectInitializer?: Partial<OperationMethod>) {
    super(`${opMethod.operation.details.csharp.name}_Validate`, mscorlib.Task());
    this.apply(objectInitializer);
    this.access = Access.Internal;
    this.async = Modifier.Async;
